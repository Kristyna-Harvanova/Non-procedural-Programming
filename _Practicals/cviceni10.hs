import Crypto.KDF.Scrypt (Parameters(r))
-- 10. cvičení 2024-04-23
--
...
insertMy :: Ord a => a -> Tree a -> Tree a
insertMy x Leaf = Node Leaf x Leaf
insertMy x (Node l y r)
    | x < y = Node (insertMy x l) y r
    | x > y = Node l y (insertMy x r)
    | x == y = Node l y r

insertSefl :: Ord a => a -> Tree a -> Tree a
insertSefl x Leaf = Node Leaf x Leaf
insertSefl x (Node l y r) = case compare x y of
    LT -> Node (insertMy x l) y r
    EQ -> Node l y r
    GT -> Node l y (insertMy x r)

delete :: Ord a => a -> Tree a -> Tree a
delete x Leaf = Leaf
delete x (Node l y r)
    | x == y = Node l (minFromTree r) (deleteMin r)
    | x < y = Node (delete x l) y r
    | X > y = Node l y (delete x r)

minFromTree :: Tree a -> a
minFromTree (Node Leaf y _) = y
minFromTree (Node l y r) = minFromTree l
...
instance Eq BoolFn where
    BoolFn a == BoolFn b = (a True == b True) && (a False == b False)
...
instance Show BoolFn where
    ...
    --     , " in f)"
    --     ]
    show (BoolFn f) = "(let f True = " ++ show (f True) ++ "; f False = " ++ show (f False) ++ " in f)"
