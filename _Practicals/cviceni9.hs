import XMonad.Util.Dzen (y)
import Codec.Archive.Tar (Entries(Fail))
import Distribution.TestSuite (OptionDescr)
import qualified GHC.RTS.Flags as Debug
-- 9. cvičení 2024-04-16
--
-- Pro připomenutí:
--
...
neboVsude :: [Bool] -> Bool
neboVsude [] = False
neboVsude (b:bs) = nebo b (neboVsude bs)
...
applyAll :: [a -> a] -> a -> a
-- applyAll [] z = z                      -- pomoci rekurze
-- applyAll (f:fs) z = f $ applyAll fs z

applyAll fns z = foldr (\f x -> f x) z fns
